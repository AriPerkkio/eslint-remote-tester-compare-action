...
// node_modules/eslint-remote-tester/dist/file-client/result-templates.js
var require_result_templates = __commonJS((exports2) => {
  "use strict";
  Object.defineProperty(exports2, "__esModule", {value: true});
  exports2.RESULT_PARSER_TO_EXTENSION = exports2.RESULT_PARSER_TO_COMPARE_TEMPLATE = exports2.RESULT_PARSER_TO_TEMPLATE = exports2.RESULTS_TEMPLATE_CI_BASE = exports2.ComparisonTypes = void 0;
  var NEW_LINE_REGEX = /\n/g;
  exports2.ComparisonTypes = ["added", "removed"];
  var RESULT_TEMPLATE_PLAINTEXT = (result) => `Rule: ${result.rule}
...
${result.error}
` : ""}`;
  var RESULT_TEMPLATE_MARKDOWN = (result) => `## Rule: ${result.rule}
...
\`\`\`
` : ""}`;
  exports2.RESULTS_TEMPLATE_CI_BASE = (result) => `Repository: ${result.repositoryOwner}/${result.repository}`;
  var COMPARISON_RESULTS_TEMPLATE_PLAINTEXT = (type, results) => `${upperCaseFirstLetter(type)}:
${results.length ? results.map(RESULT_TEMPLATE_PLAINTEXT).join("\n") : "No changes"}`;
  var COMPARISON_RESULTS_TEMPLATE_MARKDOWN = (type, results) => `# ${upperCaseFirstLetter(type)}:
${results.length ? results.map(RESULT_TEMPLATE_MARKDOWN).join("\n") : "No changes"}`;
  exports2.RESULT_PARSER_TO_TEMPLATE = {
    plaintext: RESULT_TEMPLATE_PLAINTEXT,
    markdown: RESULT_TEMPLATE_MARKDOWN
  };
  exports2.RESULT_PARSER_TO_COMPARE_TEMPLATE = {
    plaintext: COMPARISON_RESULTS_TEMPLATE_PLAINTEXT,
    markdown: COMPARISON_RESULTS_TEMPLATE_MARKDOWN
  };
  exports2.RESULT_PARSER_TO_EXTENSION = {
    plaintext: "",
    markdown: ".md"
  };
...
var ESLINT_REMOTE_TESTER_BIN = "./node_modules/.bin/eslint-remote-tester";
var INTERNAL_CONFIG = "eslint-remote-tester-compare.config.js";
var DEFAULT_CONFIG = {
  CI: true,
  compare: true,
...
var CONFIGURATION_TEMPLATE = (configuration, configurationPath) => ` // Generated by eslint-remote-tester-compare-action
...
module.exports = {
    ...
    onComplete: async function onCompare(results, comparisonResults) {
        ...
        ${configuration.onComplete ? `require(${configurationPath}).onComplete(results, comparisonResults);` : "// No onComplete detected"}
    }
};
`;
async function runTester(configLocation) {
  try {
    ...
    if (!import_fs.default.existsSync(import_path.default.resolve(configLocation))) {
      throw new Error(`Unable to find eslint-remote-tester with path ${configLocation}`);
    }
    const userProvidedConfig = require(import_path.default.resolve(configLocation));
    import_fs.default.writeFileSync(INTERNAL_CONFIG, CONFIGURATION_TEMPLATE({...userProvidedConfig, ...DEFAULT_CONFIG}, configLocation));
...
var CACHE_TEMP_LOCATION = "/tmp/.comparison-cache.json";
var CACHE_ORIGINAL_LOCATION = ".cache-eslint-remote-tester/.comparison-cache.json";
async function run() {
  try {
    const githubToken = core2.getInput("github-token", {required: true});
    const repositoryInitializeCommand = core2.getInput("repository-initialize-command", {required: true});
    const usersEslintRemoteTesterConfig = core2.getInput("eslint-remote-tester-config", {required: true});
    if (import_github.context.eventName !== "issue_comment") {
      throw new Error(`invalid eventName: (${import_github.context.eventName})`);
    }
    const comment = getComment();
    if (!comment.startsWith(TRIGGER_KEYWORD))
      return;
    core2.info("Parse PR repository");
    const octokit = import_github.getOctokit(githubToken);
    ...
    core2.info(`PR repository ${prCloneUrl} - ${prBranch}`);
    core2.info("Initializing repository");
    await import_exec2.exec(repositoryInitializeCommand);
    core2.info("#1 run of eslint-remote-tester");
    await runTester(usersEslintRemoteTesterConfig);
    core2.info("Save eslint-remote-tester cache");
    import_fs2.default.renameSync(CACHE_ORIGINAL_LOCATION, CACHE_TEMP_LOCATION);
    await import_exec2.exec(`ls -lh ${CACHE_TEMP_LOCATION}`);
    await import_exec2.exec("pwd");
    core2.info(`Checkout to ${prCloneUrl} - ${prBranch}`);
    await import_exec2.exec("git clean -df");
    await import_exec2.exec(`git remote add pr ${prCloneUrl}`);
    await import_exec2.exec("git fetch pr");
    await import_exec2.exec(`git checkout pr/${prBranch}`);
    core2.info("Initializing PR repository");
    await import_exec2.exec(repositoryInitializeCommand);
    core2.info("Restoring eslint-remote-tester cache");
    import_fs2.default.renameSync(CACHE_TEMP_LOCATION, CACHE_ORIGINAL_LOCATION);
    await import_exec2.exec(`ls -lh ${CACHE_ORIGINAL_LOCATION}`);
    await import_exec2.exec("pwd");
    core2.info("#2 run of eslint-remote-tester");
    await runTester(usersEslintRemoteTesterConfig);
    core2.info("Post results");
    const comparisonResults = core2.getInput("comparisonResults");
...
var {RESULT_PARSER_TO_COMPARE_TEMPLATE} = require_result_templates();
var COMMENT_TEMPLATE = (added, removed) => `Comparison results:
...
`;
function getComment() {
  if (!import_github.context.payload.comment)
    return "";
  return import_github.context.payload.comment.body || "";
}
run();
